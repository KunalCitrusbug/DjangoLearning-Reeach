...
class UpdateCampaign(View):
    ...
    def post(self, *args, **kwargs):
        campaign_obj = get_object_or_404(Campaign, id=kwargs.get("pk"))
        campaign_obj.name = self.request.POST.get("name")
        campaign_obj.save()
...
class UpdateSettings(View):
    def post(self, *args, **kwargs):
        ...
        campaign_obj.start_time = Time.objects.get(
            name=self.request.POST.get("start_time")
        )
        campaign_obj.end_time = Time.objects.get(name=self.request.POST.get("end_time"))
        campaign_obj.send_number = ClientPhone.objects.get(
            phone=self.request.POST.get("send_number")
        )
        campaign_obj.double_optin = self.request.POST.get("double_optin_checkbox")
        campaign_obj.double_optin_keyword = self.request.POST.get(
            "double_optin_keyword"
        )
        campaign_obj.double_optin_message = self.request.POST.get(
            "Double_optin_message"
        )
        campaign_obj.limit_multiple = self.request.POST.get("Limit_multiple_checkbox")
        campaign_obj.limit_multiple_message = self.request.POST.get(
            "Limit_multiple_message"
        )
        campaign_obj.cancellation_trigger = self.request.POST.get(
            "Cancellation_trigger_checkbox"
        )
